import pygame
from sprites.actions import TerminalCompletedAction, Q2GameAction
...
class Terminal1RoomQuestion2(Terminal1Room):
    def __init__(self, screen, player, exits, compl=False):
        super().__init__(screen, player, exits)

        if not compl:
            self.q1_terminal = place_terminal(430, 500, Q2GameAction(self.screen))
        else:
